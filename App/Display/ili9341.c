...
Display_Status_t Display_Init(void)
{
    ...
    /* Программный SWRESET ДОПОЛНИТЕЛЬНО к аппаратному импульсу на RST — не
     * лишний: CS у этой панели жёстко на GND (не управляется программно),
     * то есть контроллер слушает шину SPI ВСЕГДА, в том числе в короткое
     * окно между физической подачей питания и тем моментом, когда прошивка
     * доходит до MX_GPIO_Init() (после HAL_Init()/SystemClock_Config()) —
     * до этого момента RST/DC/SCK/MOSI ещё в POR-дефолте (вход, без
     * подтяжки), и случайный шум на шине в это окно контроллер мог
     * воспринять как команду ещё ДО нашего аппаратного сброса. SWRESET
     * здесь выполняется, когда шина уже гарантированно под управлением
     * прошивки — не зависит от того, что случилось раньше. Это и есть
     * объяснение наблюдавшегося "иногда включается только после
     * нескольких ресетов" (см. чат) — если не поможет полностью, следующий
     * шаг — аппаратный (резистор pull-down на RESX, чтобы линия сама
     * держала сброс, пока МК не взял её под контроль). */
    write_command(ILI9341_CMD_SWRESET);
    HAL_Delay(ILI9341_DELAY_AFTER_RST_MS);
...
Display_Status_t Display_SetRotation(Display_Rotation_t rotation)
{
    ...
    /* MX/MY здесь ЗЕРКАЛЬНО по отношению к тому, что было в st7789.c — и
     * даже это оказалось не всё: по факту на этой панели корректная landscape-
     * ориентация — ЧИСТЫЙ MV, без MX/MY вообще. Ход экспериментов (все три
     * варианта — MV=1 с MX/MY/ни один — образуют группу из 4 комбинаций:
     * "как есть" + горизонтальное зеркало (MX) + вертикальное зеркало (MY) +
     * поворот на 180° (MX|MY) друг относительно друга):
     *   MX|MV — зеркально по горизонтали (первая попытка, отчёт "зеркальное")
     *   MY|MV — зеркально по вертикали, т.е. "перевёрнуто" (эта попытка)
     *   MV (без MX/MY) — то, что должно остаться, если МЫ убираем оба зеркала
     * Проверено вживую только для ROTATION_0 (единственная, которую реально
     * использует Display_Init()). ROTATION_180 — 180°-поворот той же базы,
     * т.е. MX|MY|MV, по той же логике, но НЕ проверялся. ROTATION_90/270
