 ...
 * AFTER_RST/SLPOUT подняты со 120 до 300 — датащитный минимум, но
 * некоторые клоны ILI9341 на практике требуют больше именно тут
 * (это отдельная пауза от внешней, что перед вызовом Display_Init()
 * в main.c — та ведёт себя монотонно противоположно ожиданиям на
 * конкретном экземпляре, см. чат/память проекта). */
#define ILI9341_DELAY_RESET_MS     10  /* Длительность низкого уровня RESX */
#define ILI9341_DELAY_AFTER_RST_MS 300 /* Пауза после аппаратного reset    */
#define ILI9341_DELAY_SLPOUT_MS    300 /* Пауза после Sleep Out            */
