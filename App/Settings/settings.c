 ...
 *            данные+checksum, и только в конце magic пишется обратно
 *            валидным значением — см. Settings_Save(). Если питание
 *            пропало в любой момент между инвалидацией и финальной
...
bool Settings_Save(void)
{
    ...
    /* Magic сначала ИНВАЛИДИРУЕТСЯ, потом данные+checksum, и только в
     * конце magic пишется обратно ВАЛИДНЫМ значением. Раньше magic просто
     * писался последним тем же постоянным значением 0xA55A, что уже лежало
     * в EEPROM с прошлого успешного сохранения, — то есть на всё время
     * записи данных magic оставался "валидным" ещё ДО начала этого
     * Settings_Save(), и реальной защитой от потери питания посреди записи
     * был не сам magic, а случайное несовпадение checksum. Явная
     * инвалидация здесь делает гарантию настоящей и прямой: питание
     * пропало в любой момент между этой записью и финальной записью
     * валидного magic — Settings_Load() увидит невалидный magic напрямую. */
    uint8_t invalid_magic[2] = { 0x00, 0x00 };
    if (EEPROM_Write(SETTINGS_EEPROM_ADDR_MAGIC, invalid_magic, 2) != EEPROM_OK) {
        return false;
    }
