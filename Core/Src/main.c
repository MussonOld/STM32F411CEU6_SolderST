...
#define POK_WAIT_TIMEOUT_MS 2000U
  ...
  /* Небольшой запас ПОСЛЕ подтверждения Pok (или после таймаута выше) —
   * компаратор Power-Good может сработать чуть раньше, чем рельс питания
   * панели полностью устаканится; подбирается по месту. */
  HAL_Delay(50);
  /* USER CODE END Init */
