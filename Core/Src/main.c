...
int main(void)
{
   ...
   * если хоть одно потеряется (подозрение на кратковременную просадку
   * питания именно во время затяжной непрерывной DMA-посылки, без
   * программатора), s_busy никогда не сбросится, и этот while завис бы
   * НАВСЕГДА, замораживая вообще весь МК (это самое начало main(), ничего
   * после — кнопки/звук/что угодно — ещё не крутится, поэтому "завис
   * дисплей" и "завис МК" тут неотличимы снаружи). 300мс — с большим
   * запасом от реального времени заливки кадра (десятки мс), но не
   * настолько долго, чтобы задержка была заметна при обычном успешном
   * старте. Самосброс — самое простое надёжное восстановление при
   * залипшей периферии; следующий проход всегда увидит is_second_pass
   * (SFTRST), так что зацикливания в двойной пре-инициализации не будет,
   * просто ещё один быстрый перезапуск поверх. */
  {
