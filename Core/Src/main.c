...
int main(void)
{
  ...
  /* Причина сброса — максимально рано, до того как что-либо ещё могло бы
   * тронуть RCC->CSR. Эмпирически (см. чат/память проекта): дисплей НИ РАЗУ
   * не стартовал при истинном power-on (BOR/POR), но стартует 100% после
   * ЛЮБОГО последующего reset (кнопка/программатор/наш собственный
   * NVIC_SystemReset()) — независимо от источника питания (от своей LDO
   * платы или от программатора, проверено — одинаково) и независимо от
   * длины любых пауз, которые мы перебирали (0-1000 мс). Это похоже на
   * задокументированную для ILI9341 разницу между поведением при истинном
   * power-on (встают аналоговые рельсы/charge-pump'ы) и при hardware/
   * software reset (они уже заряжены) — цифровой паузой на RESX не
   * лечится. Используем это: если сброс был по питанию — после
   * Display_Init() сами вызываем NVIC_SystemReset(), эмулируя
   * подтверждённо рабочий "тёплый" сценарий. Не чинит первопричину
   * (стоит при случае посмотреть VCI осциллографом), но делает cold-boot
   * невидимо рабочим для пользователя. */
  bool was_power_on_reset = __HAL_RCC_GET_FLAG(RCC_FLAG_PORRST) || __HAL_RCC_GET_FLAG(RCC_FLAG_BORRST);
  ...
  /* Это был реальный power-on, а не последующий reset — см. комментарий в
   * USER CODE BEGIN Init. Самоперезагрузка эмулирует подтверждённо рабочий
   * тёплый сценарий; для пользователя это доли секунды до появления
   * картинки на экране, а не "не запустился". */
