...
int main(void)
{
  ...
  /* Диагностика: пауза на прогрев питания панели, после раннего assert-low
   * RST/DC. 250 мс — стабильно работает после тёплого ресета. 500 мс (в
   * двух разных местах кода) — стабильно ломает даже тёплый ресет,
   * heartbeat при этом мигает ровно (МК не виснет, дело в самом
   * дисплее). Тест: 1000 мс — если заработает, у этого экземпляра есть
   * узкое "грязное" окно на его собственном рельсе питания в районе
   * 300-600 мс, а не проблема "чем дольше в сбросе — тем хуже". */
  HAL_Delay(1000);
  /* USER CODE END Init */
