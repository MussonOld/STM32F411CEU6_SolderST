...
int main(void)
{
  ...
  /* Диагностика незапуска ILI9341 при холодном включении (~0% success),
   * при этом быстрый двойной NRST-ресет запускается в ~10% случаев —
   * прошивка идентична в обоих сценариях, значит дело не в init-
   * последовательности, а во внешнем состоянии, которое НЕ сбрасывается
   * NRST: питание/reset-линия самого дисплея ещё не готовы к моменту,
   * когда MCU (после NRST уже готовый быстрее) трогает RST/SPI.
   * Задержка стоит здесь намеренно — до MX_GPIO_Init()/MX_SPI1_Init(),
   * то есть раньше, чем что-либо тронет линии дисплея, а не только
   * перед Display_Init() (там уже поздно, GPIO/SPI успевают
   * переключиться раньше, чем рельс питания панели устаканится).
   * Значение подобрать экспериментально сериями cold power-cycle;
   * после подтверждения — либо снизить до минимально нужного,
   * либо убрать совсем, если корень в другом. */
  HAL_Delay(250);
  /* USER CODE END Init */
